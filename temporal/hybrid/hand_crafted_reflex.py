...
def train(policy='TD3', seed=0, start_timesteps=25e3, eval_freq=5e3, max_timesteps=1e5,
          ...
          delayed_env=False, env_name='InvertedPendulum-v2', parent_response_rate=0.04, reflex_threshold=0.15,
          reflex_force_scale=1.0):

    max_force = g_ratio * 9.81
    eval_policy = eval_policy_increasing_force_hybrid_reflex
    arguments = ["hand_crafted_reflex", policy, env_name, seed, jit_duration, g_ratio, response_rate, catastrophe_frequency,
                 delayed_env, parent_response_rate, reflex_threshold, reflex_force_scale]
    ...
    policy = utils.HandCraftedReflex(env.observation_space, reflex_threshold, reflex_force_scale).to(device)
    ...
    avg_reward, _, _, _ = eval_policy(policy, parent_policy, env_name, parent_max_action, eval_episodes=10,
                                      ...
                                      delayed_env=delayed_env, parent_steps=parent_steps, zero_reflex=False)
    ...
    for t in range(int(max_timesteps)):
        ...
        if (t + 1) % eval_freq == 0:
            ...
            avg_reward, _, _, _ = eval_policy(policy, parent_policy, env_name, parent_max_action, eval_episodes=10,
                                                 ...
                                                 parent_steps=parent_steps, zero_reflex=False)
